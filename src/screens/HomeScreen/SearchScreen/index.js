...
function SearchScreen({ navigation }) {
  ...
  const searchSubject = async (value) => {
    ...
    try {
      ...
      const response = await subjectAPI.searchSubjectByNameAndDes(
        { searchValue: value },
        accessToken
      );
  ...
  const debounceDropDown = useCallback(
    debounce((nextValue) => searchSubject(nextValue), 1000),
    []
  );
    ...
    <View style={{ height: "100%", backgroundColor: "#169d9e" }}>
      ...
      <View style={{ marginTop: 10 }}>
        ...
        {searchResult === null && searchValue !== null && (
          <Text style={styles.notFoundSubject}>
            Not found subject with keyword: {searchValue}
          </Text>
        )}
      </View>
